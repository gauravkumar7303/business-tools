    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.value && data.value.length > 0) {
          ...
          // Check if directUnitCost field exists
          const firstLine = data.value[0];
          if (firstLine.directUnitCost !== undefined) {
            console.log(`✅ directUnitCost field found in API: ${firstLine.directUnitCost}`);
          } else {
            console.log('⚠️ directUnitCost field NOT found in API response');
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.value && data.value.length > 0) {
            ...
            // Check for directUnitCost field
            const firstLine = data.value[0];
            if (firstLine.directUnitCost !== undefined) {
              console.log(`✅ directUnitCost field found: ${firstLine.directUnitCost}`);
            }
    ...
    if (realLines.length > 0) {
      ...
      return Response.json({
        ...
        _debug: {
          ...
          cleanWoNo: cleanWoNo,
          originalWoNo: woNo,
          sampleLine: processedLines.length > 0 ? processedLines[0] : null
        }
      }, {
        status: 200,
        headers: {
...
// Process Business Central WO lines with proper field mapping - FIXED FOR YOUR API
function processBusinessCentralLines(lines, woNumber) {
  console.log(`\n🔄 Processing ${lines.length} Business Central WO lines...`);
  
  return lines.map((line, index) => {
    console.log(`\n🔍 Processing line ${index + 1}:`);
    console.log('📋 Raw line data:', JSON.stringify(line, null, 2));
    
    // Handle multiple possible field names from BC API
    // Your API uses: field("documentNo"; Rec."Document No.")
    const documentNo = line.documentNo || line['Document No.'] || line.Document_No || woNumber;
    
    // Your API uses: field("lineNo"; Rec."Line No.")
    const lineNo = line.lineNo || line.Line_No || line['Line No.'] || (index + 1);
    
    // Your API uses: field("no"; Rec."No.")
    const itemNo = line.no || line.itemNo || line.No || line['No.'] || `ITEM-${String(index + 1).padStart(3, '0')}`;
    
    // Your API uses: field("description"; Rec.Description)
    const description = line.description || line.Description || 'Work Order Item';
    
    // Your API uses: field("quantity"; Rec.Quantity)
    const quantity = parseFloat(line.quantity || line.Quantity || line.qty || line.Qty || 1);
    
    // ✅ IMPORTANT: Your API uses: field("directUnitCost"; Rec."Direct Unit Cost")
    let unitCost = 0;
    let unitCostSource = 'not_found';
    
    // 1. First priority: "directUnitCost" from your API Page 50174
    if (line.directUnitCost !== undefined && line.directUnitCost !== null) {
      unitCost = parseFloat(line.directUnitCost);
      unitCostSource = 'directUnitCost_field';
      console.log(`✅ Found unit cost in "directUnitCost" field: ${unitCost}`);
    }
    // 2. Try other possible field names for Direct Unit Cost
    else if (line.direct_unit_cost !== undefined && line.direct_unit_cost !== null) {
      unitCost = parseFloat(line.direct_unit_cost);
      unitCostSource = 'direct_unit_cost_field';
      console.log(`✅ Found unit cost in "direct_unit_cost" field: ${unitCost}`);
    }
    // 3. Try other possible field names
    else {
      const possibleUnitCostFields = [
        'Direct_Unit_Cost', 'DirectUnitCost', 'Direct Unit Cost',
        'unitCost', 'Unit_Cost', 'UnitCost', 'unit_cost',
        'cost', 'Cost', 'unitPrice', 'UnitPrice', 'Unit_Price'
      ];
      
      for (const field of possibleUnitCostFields) {
        if (line[field] !== undefined && line[field] !== null) {
          unitCost = parseFloat(line[field]);
          unitCostSource = field;
    ...
    // 4. If unit cost is still 0, try to calculate from line amount and quantity
    if (unitCost === 0) {
      // Your API uses: field("lineAmount"; Rec."Line Amount")
      const lineAmount = parseFloat(line.lineAmount || line['Line Amount'] || line.amount || line.Amount || line.Line_Amount || 0);
      
      if (lineAmount > 0 && quantity > 0) {
        unitCost = lineAmount / quantity;
        unitCostSource = 'calculated_from_lineAmount';
        console.log(`📊 Calculated unit cost: ${lineAmount} / ${quantity} = ${unitCost}`);
      }
    }
    
    // Your API uses: field("lineAmount"; Rec."Line Amount")
    const lineAmount = parseFloat(line.lineAmount || line['Line Amount'] || line.amount || line.Amount || line.Line_Amount || (quantity * unitCost) || 0);
    
    // Your API uses additional fields
    const detailedSpecification = line.Detailed_Specification_1 || line['Detailed Specification 1'] || '';
    const itemType = line.Type || line.type || '';
    const gstGroupCode = line.gstGroupCode || line['GST Group Code'] || line.GST_Group_Code || '';
    const hsnSacCode = line.hsnSacCode || line['HSN/SAC Code'] || line.HSN_SAC_Code || '';
    
    const unitOfMeasure = line.unitOfMeasure || line['Unit of Measure'] || line.Unit_of_Measure_Code || line.Unit_of_Measure || 'PCS';
    const location = line.location || line.Location || line.locationCode || line['Location Code'] || 'MAIN';
    const status = line.status || line.Status || 'Planned';
    
    console.log(`📝 Processed WO Line ${lineNo}:`);
    console.log(`   Item: ${itemNo} - ${description}`);
    console.log(`   Quantity: ${quantity}, Unit Cost: ${unitCost} (source: ${unitCostSource})`);
    ...
    return {
      ...
      description,
      quantity,
      unitCost, // ✅ अब unit cost properly capture होगा
      lineAmount,
      unitOfMeasure,
      location,
      status,
      ...
      _source: 'business_central',
      _debug: {
        unitCostSource: unitCostSource,
        rawDirectUnitCost: line.directUnitCost,
...
// Generate realistic WO test data WITH UNIT COST
function generateRealisticTestData(woNumber) {
  console.log(`🎨 Creating realistic test lines for WO: ${woNumber}`);
  ...
  return [
    {
      ...
      status: 'Released',
      documentNo: woNumber,
      detailedSpecification: 'Steel frame for main assembly',
      itemType: 'Inventory',
      gstGroupCode: '18%',
      hsnSacCode: '8431',
      _source: 'test_data'
    },
    {
      ...
      status: 'Released',
      documentNo: woNumber,
      detailedSpecification: 'Control panel with PLC',
      itemType: 'Inventory',
      gstGroupCode: '18%',
      hsnSacCode: '8537',
      _source: 'test_data'
    },
    {
      ...
      status: 'Planned',
      documentNo: woNumber,
      detailedSpecification: 'Hydraulic cylinders and valves',
      itemType: 'Inventory',
      gstGroupCode: '18%',
      hsnSacCode: '8412',
      _source: 'test_data'
    },
    {
      ...
      status: 'Released',
      documentNo: woNumber,
      detailedSpecification: 'Bearings for rotating parts',
      itemType: 'Inventory',
      gstGroupCode: '18%',
      hsnSacCode: '8482',
      _source: 'test_data'
    },
    {
      ...
      status: 'Planned',
      documentNo: woNumber,
      detailedSpecification: 'Protective coating material',
